    ...
    while (t--)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            if (vec[i] == 0)
            {
                for (int j = 1; j <= m; j++)
                {
                    dp[i][j] = (j - 1 >= 0 ? dp[i - 1][j - 1] : 0) % MOD + dp[i - 1][j] % MOD + (j + 1 <= m ? dp[i - 1][j + 1] : 0) % MOD;
                }
            }
            else
            {
                dp[i][vec[i]] = (vec[i] - 1 >= 0 ? dp[i - 1][vec[i] - 1] : 0) % MOD + dp[i - 1][vec[i]] % MOD + (vec[i] + 1 <= m ? dp[i - 1][vec[i] + 1] : 0) % MOD;
            }
        }
        int ans = 0;
        for (int i = 1; i <= m; i++)

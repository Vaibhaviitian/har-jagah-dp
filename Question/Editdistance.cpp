...
void coderaryan()
{
    ...
    string a, b;
    cin >> a >> b;
    vector<vector<int>> va(a.size(), vector<int>(b.size()));
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (i == 0 && j == 0)
            {
                // bcoz already base case were defined
                continue;
            }
            if (a[i] == b[j])
            {
                va[i][j] = min(i - 1 >= 0 ? va[i - 1][j] + 1 : 2e9, min(j - 1 >= 0 ? va[i][j - 1] + 1 : 2e9, i - 1 >= 0 && j - 1 >= 0 ? va[i - 1][j - 1] : 2e9));
            }
            else
            {
                va[i][j] = min(i - 1 >= 0 ? va[i - 1][j] + 1 : 2e9, min(j - 1 >= 0 ? va[i][j - 1] + 1 : 2e9, i - 1 >= 0 && j - 1 >= 0 ? va[i - 1][j - 1] + 1: 2e9));
            }
        }
    }
